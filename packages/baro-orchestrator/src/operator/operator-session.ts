    ...
    const functions: HostFunction[] = [
        {
            ...
            parameters: {
                ...
                properties: {
                    ...
                    base: {
                        type: "string",
                        description:
                            "Optional git ref (e.g. main) to start the run from in its own worktree; the checkout is not touched and the run does not wait for other runs.",
                    },
                },
                required: ["goal"],
            ...
            invoke: async (args) => {
                const { goal, cwd, base } = args as { goal?: unknown; cwd?: unknown; base?: unknown }
                ...
                const baseRef = typeof base === "string" && base.trim() ? base.trim() : undefined
                const dirty = baseRef ? [] : await uncommittedPaths(target)
                if (dirty.length) {
                ...
                const { run, behind } = registry.delegate(goal.trim(), target, baseRef)
                ui.runsChanged(registry.rows())
                if (behind) {
                ...
                return `${run.id} started in the background (cwd ${run.cwd}${baseRef ? `, from ${baseRef}` : ""}). Intake and planning take a few minutes before stories start; ask run_status for progress.`
            },
        },
        {
            name: "runs",
...
export function systemPrompt(cwd: string): string {
    return `You are baro's operator: a coding agent working inside the repository at ${cwd}, with baro as your back office. baro runs multi-story goals as a verified pipeline (architect, planner, parallel coding agents, independent per-story review, verification) in the background through the \`delegate\` tool.
...
- direct: one component you can finish in roughly ten minutes, clear intent, typically up to eight files. Do it yourself like a normal coding agent: edit, run the relevant tests, say what changed. Finish the whole thing in one turn; do not split a job to stay under a limit. Do not commit unless asked. The host refuses the eleventh file in one turn as a runaway-scope guard; if you hit it, delegate the rest.
- delegate: work whose scope you cannot see to the end, changes across several modules or owned by different people, anything that needs independent review, or anything the user asks to run through baro. baro has a fixed cost: intake, architect and goal contract take ten to fifteen minutes before the first story starts, whatever the size. So for a goal you could finish directly in a few minutes, say so and offer the choice in one line ("direct in ~3 min, or baro in ~15 with review?") instead of delegating by default. When you delegate, call \`delegate\` with a precise, self-contained goal (what, where, constraints, how to verify); baro's planner reads only that text. \`delegate\` refuses while the working tree has uncommitted changes: baro's agents work from the last commit and baro merges finished stories into this very checkout, so uncommitted edits block a story from landing. Before delegating, get the tree clean with the user's word (commit, on a branch if the work must not ride along; or stash). One checkout serves one run's branch: without \`base\`, an independent goal waits until the active run in this repository finishes. To run it now, delegate it with \`base\` (e.g. "main"): it starts from that ref in its own worktree, never touches this checkout, and skips the uncommitted-changes check. While a run is active in this repository, do not edit files in the checkout; answer, plan, or delegate instead. It returns at once with a run id and the run continues in the background. Do not poll in a loop. Tell the user the run id and one sentence on what to expect, then keep talking.

When asked what the agents are doing, call \`run_status\` (or \`runs\`) and summarize plainly: phase, stories done of total, last activity, recent milestones. When a run finishes you receive a message starting with [baro]; report the outcome. If the user overrides your altitude ("just do it yourself" / "send it to baro"), follow them.
