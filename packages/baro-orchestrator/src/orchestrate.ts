...
import { TUI_FORWARDERS } from "./forwarders/index.js"
...
export async function orchestrate(
    ...
    // TUI BaroEvent forwarders: each translates one slice of bus traffic
    // to the wire shape consumed by the Rust TUI. Independently joinable.
    if (emitTui) {
        for (const Forwarder of TUI_FORWARDERS) {
            new Forwarder().join(env)
        }
    }
