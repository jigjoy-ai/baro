...
import type { WorktreeManager } from "../integration/worktree.js"
import {
...
export class StoryFactory extends BaseObserver {
    ...
    private collaborationInstructions(
        ...
        return [
            ...
            // Working around a false premise in silence is what leaves the
            // contract asserting it for the rest of the run, with the Critic
            // judging every later story against text this repository cannot
            // satisfy.
            `- If the architecture contract states something about this repository or runtime that is FALSE (a command that does not do what it claims, an import that fails, a symbol that does not exist), do not just work around it — withdraw the claim with the command that refutes it: ${command} emit ${capability} --kind dispute --claim ${JSON.stringify("WHAT THE CONTRACT STATES")} --command ${JSON.stringify("THE COMMAND YOU RAN")} --output ${JSON.stringify("ITS OUTPUT")} [--obligation O-015]. The decisions stand; only the false claim is withdrawn, and the Critic then judges that criterion by its purpose instead of an impossible literal.`,
