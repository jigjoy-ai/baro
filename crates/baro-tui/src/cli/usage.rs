...
pub const CONNECT_USAGE: &str = concat!(
    ...
    "  --token <rt_…>         runner token to pair with, one-shot only — never stored in argv;\n",
    "                         resolved from $BARO_RUNNER_TOKEN, then $RUNNER_TOKEN, else the\n",
    "                         0600 token file baro connect --install-service writes\n",
